-- Allow decimal values (e.g. 5.5, 6.8) for initial assessment scores
-- Previously all score columns were INTEGER, restricting to whole numbers only

-- 1. Drop view that depends on overall_rating / episodes columns
DROP VIEW IF EXISTS consolidated_cms_view;

-- 2. episodes.initial_assessment: INTEGER → DECIMAL(3,1)
ALTER TABLE episodes
  DROP CONSTRAINT IF EXISTS episodes_initial_assessment_range;
ALTER TABLE episodes
  ALTER COLUMN initial_assessment TYPE DECIMAL(3,1) USING initial_assessment::DECIMAL(3,1);
ALTER TABLE episodes
  ADD CONSTRAINT episodes_initial_assessment_range
    CHECK (initial_assessment IS NULL OR (initial_assessment >= 1 AND initial_assessment <= 10));

-- 3. call_reports.overall_rating: INTEGER → DECIMAL(3,1)
ALTER TABLE call_reports
  ALTER COLUMN overall_rating TYPE DECIMAL(3,1) USING overall_rating::DECIMAL(3,1);

-- 4. Recreate consolidated_cms_view (identical to current definition)
...
episode_stats AS (
    SELECT
        ...
        MIN(COALESCE(e.original_submission_date::timestamptz, e.created_at)) AS first_episode_received_at,
        MAX(COALESCE(e.original_submission_date::timestamptz, e.created_at)) AS last_episode_received_at,
