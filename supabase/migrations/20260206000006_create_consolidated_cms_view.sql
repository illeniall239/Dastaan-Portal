-- Migration: Create Consolidated CMS View
-- Description: Aggregates all script data, multi-layer evaluations, and episode aging into a single view
-- Date: 2026-02-06 (updated 2026-02-07 with episode aging)
...
WITH evaluation_summary AS (
    SELECT
        ...
        AVG(ef.overall_assessment_score) AS avg_overall
...
SELECT
    ...
    -- Evaluations (explicitly listed to avoid call_report_id ambiguity)
    te.content_conflict,
...
COMMENT ON VIEW consolidated_cms_view IS 'Master view for the Consolidated Content Management Dashboard (CMS) with episode aging data';
