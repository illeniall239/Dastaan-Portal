-- Migration: Fix Consolidated CMS View - Per-Group Averages & Counts
-- Description:
--   1. Use AVG(average_score) instead of AVG(overall_assessment_score) for *_overall columns
--   2. Filter to only submitted evaluations (submitted_at IS NOT NULL)
--   3. Add per-group evaluator counts (content_eval_count, programming_eval_count, management_eval_count)
-- Date: 2026-02-07
...
COMMENT ON VIEW consolidated_cms_view IS 'Master view for the Consolidated CMS with per-group evaluation averages and counts';
