...
export default async function NewGcmNegotiationPage() {
  ...
  if (!userData || !["gcm", "admin"].includes(userData.role)) {
    redirect("/dashboard");
  }

  const hasGlobalAccess = userData.role && ["admin", "management"].includes(userData.role);

  // Fetch approved stories without contract terms, joining linked call report
  ...
  return (
    <div className="mobile-container mobile-section">
      <div className="mobile-header-spacing">
        <BackButton fallbackHref="/gcm/contract-terms" className="mb-4" />
