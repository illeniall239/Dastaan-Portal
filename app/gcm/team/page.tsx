...
export default async function GcmTeamPage() {
              ...
              return (
                <div key={member.id} className="space-y-3">
                  ...
                  {memberReports.length === 0 ? (
                    <Card><CardContent className="py-6 text-center text-sm text-muted-foreground italic">No one-liners logged yet</CardContent></Card>
                  ) : (
                        ...
                        <Card key={cr.id} className="overflow-hidden">
                            ...
                            <div className="flex items-start justify-between gap-3">
                              <div className="space-y-1 min-w-0">
                                <div className="flex items-center gap-2 flex-wrap">
                                  <CardTitle className="text-base">{cr.working_title || 'Untitled'}</CardTitle>
                                  {cr.call_report_id && (
                                    <Badge variant="outline" className="text-[10px] font-mono shrink-0">{cr.call_report_id}</Badge>
                                  )}
                                </div>
                                {cr.logline && <p className="text-sm text-muted-foreground line-clamp-2">{cr.logline}</p>}
                              </div>
                              {cr.overall_rating != null && (
                                <div className={`flex items-center gap-1 px-2.5 py-1 rounded-lg border text-sm font-bold shrink-0 ${getRatingColor(cr.overall_rating)}`}>
                                  <Star className="h-3.5 w-3.5" />{cr.overall_rating}/10
                                </div>
                              )}
                            </div>
                          </CardHeader>
                          <CardContent className="pt-0 space-y-3">
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-2 text-sm">
                              {cr.category && <div><span className="text-muted-foreground text-xs">Source</span><p className="font-medium truncate">{formatCategory(cr.category)}</p></div>}
                              {cr.genre && cr.genre.length > 0 && <div><span className="text-muted-foreground text-xs">Genre</span><p className="font-medium truncate">{cr.genre.join(', ')}</p></div>}
                              {cr.content_type && <div><span className="text-muted-foreground text-xs">Content Type</span><p className="font-medium">{cr.content_type}</p></div>}
                              {cr.slot && <div><span className="text-muted-foreground text-xs">Target Slot</span><p className="font-medium">{cr.slot}</p></div>}
                            </div>
                            {cr.remarks && <div className="text-sm"><span className="text-muted-foreground text-xs">Additional Notes</span><p className="text-gray-700 line-clamp-3">{cr.remarks}</p></div>}
                            {cr.next_steps && <div className="text-sm"><span className="text-muted-foreground text-xs">Next Steps</span><p className="text-gray-700 line-clamp-3">{cr.next_steps}</p></div>}
                            {cr.episodes.length > 0 && (
                              <div className="text-sm">
                                <div className="flex items-center gap-1.5 mb-1">
                                  <Film className="h-3.5 w-3.5 text-muted-foreground" />
                                  <span className="text-muted-foreground text-xs">{cr.episodes.length} Episode{cr.episodes.length !== 1 ? 's' : ''} Linked</span>
                                </div>
                                <div className="flex flex-wrap gap-1.5">
                                  {cr.episodes.map(ep => (
                                    <Badge key={ep.id} variant="secondary" className="text-xs">Ep {ep.episode_number}{ep.title ? `: ${ep.title}` : ''}</Badge>
                                  ))}
                                </div>
                              </div>
                            )}
                            <div className="flex items-center gap-2 pt-1 text-xs text-muted-foreground border-t">
                              ...
                              {cr.status && (<><span className="mx-1">·</span><Badge variant="outline" className="text-[10px]">{cr.status.replace(/_/g, ' ')}</Badge></>)}
                            </div>
                          </CardContent>
                        </Card>
                      ))}
      ...
      <div className="container mx-auto p-6">
        <Card>
          <CardContent className="py-8 text-center">
            <p className="text-muted-foreground">Unable to load team data. You may not have a team assigned yet.</p>
