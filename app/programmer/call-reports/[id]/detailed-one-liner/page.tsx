...
export default async function ProgrammerCallReportDetailedOneLinerPage({
  params,
}: {
  ...
  if (!userData || !["programmer", "management", "admin"].includes(userData.role)) {
    redirect("/dashboard");
  }
  ...
  if (callReportError || !callReport) {
    ...
    redirect("/programmer/call-reports");
  }
  ...
  const canEdit =
    ...
    (detailedOneLiner.created_by === user.id ||
      ["management", "admin"].includes(userData.role));
    ...
    <div className="mobile-container mobile-section">
      <div className="mobile-header-spacing">
        ...
        <div className="flex items-center gap-4 mb-6">
          <BackButton fallbackHref={`/programmer/call-reports/${id}`} variant="outline" />
          {canEdit && detailedOneLiner && (
            <Button asChild variant="default" size="sm">
              <Link href={`/programmer/detailed-one-liner/${detailedOneLiner.id}/edit`}>
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2 text-sm text-muted-foreground">
            <span>One-Liner: {callReport.call_report_id}</span>
            <span>•</span>
