...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (error) {
      logger.error(`Error fetching approved stories: ${error instanceof Error ? error.message : String(error)}`);
      return NextResponse.json(
    ...
    logger.error(`Error in GET /api/negotiations/approved-stories: ${error instanceof Error ? error.message : String(error)}`);
    return NextResponse.json(
