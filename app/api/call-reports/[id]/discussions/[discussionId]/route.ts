...
/**
 * DELETE /api/call-reports/[id]/discussions/[discussionId]
 * Delete a discussion message (own messages, or admin/management)
 */
export async function DELETE(
  ...
  const callReportValidation = idParamSchema.safeParse({ id });
  const discussionValidation = idParamSchema.safeParse({ id: discussionId });
  if (!callReportValidation.success || !discussionValidation.success) {
  ...
  try {
    ...
    const { data: discussion } = await supabase
      ...
      .select("id, user_id, call_report_id")
      .eq("id", discussionId)
    ...
    if (deleteError) {
      logger.error("Error deleting discussion message:", deleteError);
    ...
    logger.error("Error in DELETE /api/call-reports/[id]/discussions/[discussionId]:", error);
