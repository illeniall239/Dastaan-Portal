...
export function EvaluatorPipelineEpisodes({
  dramas,
  episodesByDrama,
  evaluatorsByEpisode,
}: EvaluatorPipelineEpisodesProps) {
  ...
  const handleEventAnalysisClick = (callReportId: string, dramaTitle: string, e: React.MouseEvent) => {
    e.stopPropagation(); // Prevent drama card from toggling
  ...
  const handleEpisodeClick = (episodeId: string, episodeNumber: number, dramaTitle: string) => {
    ...
    // Show evaluator details in modal
    const tableData = [
